...
class FaceDetectorMovidiusMTCNN:
    ...
    def __init__(self, movidius_id_pnet=0, movidius_id_onet=1):
        ...
        if movidius_id_pnet != movidius_id_onet and len(devices) < max(movidius_id_pnet, movidius_id_onet)+1:
        ...
        self.PNet_fifo_in, self.PNet_fifo_out = self.PNet.allocate_with_fifos(self.PNetDevice, pgraphfile)
        ...
        self.ONet_fifo_in, self.ONet_fifo_out = self.ONet.allocate_with_fifos(self.ONetDevice, ographfile)
    ...
    def _detect_face(self, img, threshold=[0.405, 0.8473], factor=0.709):
        ...
        self.PNet.queue_inference_with_fifo_elem(self.PNet_fifo_in, self.PNet_fifo_out, img_y[0].astype(np.float32), None)
        out, userobj = self.PNet_fifo_out.read_elem()
        ...
        if numbox > 0:
            ...
            for k in range(tempimg1.shape[0]):
                ...
                self.ONet.queue_inference_with_fifo_elem(self.ONet_fifo_in, self.ONet_fifo_out, tempimg1[k].astype(np.float32), None)
                tempout, userobj = self.ONet_fifo_out.read_elem()
                out[k, :] = tempout[:6]
